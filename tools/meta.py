...
@with_timeout("meta")
def get_meta_performance(ad_account_id: str, date_start: str, date_end: str, metrics: list) -> dict:
    """
    ...
    try:
        ...
        return error("meta", "API_ERROR", f"FacebookRequestError: {e.api_error_message()}")
...
def get_meta_active_ad_urls(ad_account_id: str) -> dict:
    ...
    Returns:
        Lista de ads con ad_id, ad_name, adset_name, campaign_name y url_destino
    """
    try:
        account = AdAccount(f"act_{ad_account_id}")
        ...
        for ad in ads:
            ad_data = {
                ...
                "destination_url": _extract_url_from_creative(ad.get("creative", {})),
            }
        ...
        campaign_list = [
            {
                ...
                "daily_budget_eur": round(float(c["daily_budget"]) / 100, 2) if c.get("daily_budget") else None,
                "lifetime_budget_eur": round(float(c["lifetime_budget"]) / 100, 2) if c.get("lifetime_budget") else None,
            }
