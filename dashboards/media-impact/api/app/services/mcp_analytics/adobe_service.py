...
class AdobeAnalyticsService(AnalyticsService):
    ...
    async def list_accounts(self) -> List[GAAccount]:
        if self.client_id == "adobe-temp":
            if self.tenant_id == "client_a":
                return [
                    GAAccount(name="companies/client_a_id", display_name="Client A", account_id="client_a_id")
                ]
            return [
    ...
    async def list_properties(self, account_id: Optional[str] = None) -> List[GAProperty]:
        if self.client_id == "adobe-temp":
            if self.tenant_id == "client_a" or account_id == "client_a_id":
                return [
                    GAProperty(name="vrs_client_a_suite", display_name="Client A Suite", property_id="vrs_client_a_suite", parent="accounts/client_a_id")
                ]
            return [
    ...
    async def list_segments(self, report_suite_id: str) -> List[Dict[str, Any]]:
        ...
        """
        if self.client_id == "adobe-temp":
            if self.tenant_id == "client_a" or report_suite_id == "vrs_client_a_suite":
                return [
                    {"id": "seg_1", "name": "Client A SEM"},
                    {"id": "seg_2", "name": "Content Library"},
                    {"id": "seg_3", "name": "Client A SEO"},
