...
def create_line_item_sdf(
    ...
    bid_eur: float = 1.0, max_budget_eur: float = _MAX_BUDGET_LI_EUR,
    max_bid_eur: float = _MAX_BID_LI_EUR,
    ...
    if bid_eur and bid_eur > max_bid_eur:
        return {
            "status": "error",
            "error": (
            ...
            "data": {"bid_eur": bid_eur, "guardrail_max_eur": max_bid_eur},
        }
...
def main() -> None:
    ...
    parser.add_argument("--max-budget", type=float, default=_MAX_BUDGET_LI_EUR,
                        help=f"Guardrail maximo presupuesto LI en EUR (defecto {_MAX_BUDGET_LI_EUR})")
    ...
    parser.add_argument("--max-bid-guardrail", type=float, default=_MAX_BID_LI_EUR,
                        help=f"Guardrail maximo bid en EUR (defecto {_MAX_BID_LI_EUR})")
    ...
    result = create_line_item_sdf(
        ...
        max_bid_eur=args.max_bid_guardrail,
