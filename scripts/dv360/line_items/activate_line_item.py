"""
scripts/dv360/line_items/activate_line_item.py
Activa un Line Item en DV360.

Uso:
    python scripts/dv360/line_items/activate_line_item.py \
        --client vidal-vidal \
...
def activate_line_item(
    ...
    """
    Activa un Line Item en DV360 (ENTITY_STATUS_ACTIVE).

    Args:
        client_id: ID del cliente
        line_item_id: ID del Line Item a activar
        dry_run: si True, muestra la accion sin ejecutarla
    """
    advertiser_id = get_advertiser_id(client_id)

    action_msg = (
        f"Activar Line Item {line_item_id} "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Activa un Line Item en DV360.")
    parser.add_argument("--client", required=True, help="ID del cliente (ej. vidal-vidal)")
    parser.add_argument("--line-item-id", required=True, help="ID del Line Item a activar")
    parser.add_argument("--dry-run", action="store_true", help="Simula la accion sin ejecutarla")

    args = parser.parse_args()

    result = activate_line_item(
