"""
scripts/dv360/insertion_orders/pause_io.py
Pausa un Insertion Order en DV360.

Uso:
    python scripts/dv360/insertion_orders/pause_io.py \
        --client vidal-vidal \
        --io-id 123456789 \
        --dry-run
...
def pause_io(
    ...
    Nota: pausar un IO pausa automaticamente todos sus Line Items activos.
    El script advierte de esto antes de confirmar.

    Args:
        client_id: ID del cliente
        io_id: ID del Insertion Order a pausar
        dry_run: si True, muestra la accion sin ejecutarla
    """
    advertiser_id = get_advertiser_id(client_id)

    action_msg = (
        f"Pausar Insertion Order {io_id} "
        f"(advertiser {advertiser_id}, cliente {client_id}). "
        "ATENCION: pausar un IO pausa automaticamente todos sus Line Items activos."
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pausa un Insertion Order en DV360.")
    parser.add_argument("--client", required=True, help="ID del cliente")
    parser.add_argument("--io-id", required=True, help="ID del Insertion Order a pausar")
